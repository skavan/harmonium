...
<div class="space-y-4">
  <p class="m-0 text-xs text-dim">
    Snippets are captured with ⤴ <b>Export snippet</b> — on an
    activity's Setup or State block, or a preset row's ⋯ menu — and
    replayed with ⤵ <b>Import snippet</b> on any compatible block.
    They live in this browser — global across every workspace, and
    reseeds never touch them.
  </p>
  {#each Object.entries(SNIPPET_TYPES) as [type, label] (type)}
    <div>
      ...
      <div class="space-y-2">
        ...
        {:else}
          <p class="m-0 text-xs text-dim">None yet — hit ⤴ Export snippet on a block to capture one.</p>
        {/each}
      </div>
    </div>
  {/each}
