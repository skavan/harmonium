# Astrion Remote Key Map (v2)

GENERATED from `key_mapper.zip` — do not hand-edit; rerun
`python ../gen-map-docs.py v2` after mappings change.
...
| Green (F9) press | 139 | `KEYCODE_F9` | 67 | Open KISS Launcher | `fr.neamar.kiss` | — | global |
| Blue (F10) press | 140 | `KEYCODE_F10` | 68 | Shell: Wireless ADB ON + Play sound: USBConnect.ogg | — | — | global |
...
| Red (F8) long-press | 138 | `KEYCODE_F8` | 66 | Android Go back | — | — | global |
| Blue (F10) long-press | 140 | `KEYCODE_F10` | 68 | Shell: Wireless ADB OFF + Play sound: USBDisconnect.ogg | — | — | global |
| Yellow (F11) long-press | 141 | `KEYCODE_F11` | 87 | expand_quick_settings | — | — | global |
...
## Shell commands (decoded)

The `Shell:` actions in the table run these commands. Key Mapper stores them base64-encoded; they are decoded here so you can retype them when building the mappings by hand — in Key Mapper add a **Shell command** action and paste the line verbatim.

### Blue (F10) press — Wireless ADB ON

```sh
sh -c 'if [ "$(getprop service.adb.tcp.port)" != "5555" ]; then setprop service.adb.tcp.port 5555; setprop ctl.restart adbd; fi'
```

### Blue (F10) long-press — Wireless ADB OFF
...
- **The colour keys are consumed by Key Mapper, not passed to Harmonium** (they never reach the webview): Red opens Fully Kiosk Browser (long-press: goes back); Green opens KISS Launcher; Blue enables wireless ADB (long-press: disables wireless ADB); Yellow opens Key Mapper (long-press: expand_quick_settings).
