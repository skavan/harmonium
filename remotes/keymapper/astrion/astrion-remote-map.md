...
- **The color keys are app launchers, not Harmonium keys**: Red opens Fully (the road home), Green the File Manager, Blue HaRemote, Yellow Key Mapper (long-press: the Android browser). KeyMapper consumes them, so they never reach the webview — delete those rules if you want them bindable in the Studio instead.
- The F4–F7 glyph row is deliberately NOT KeyMapper-mapped — those F-keys reach the webview raw; the astrion profile names them `light` / `cover` / `music` / `climate`, astrion2 names them transport. Bind what they do in the Studio: Page settings → Keys. See docs/cookbook/hardware-keys.md.
- The hold gestures land on: `]` = hold-Back, `=` = hold-Home, `F12` = hold-Power (All Off) — the engine's hold vocabulary, chosen so no typeable character carries a gesture.
