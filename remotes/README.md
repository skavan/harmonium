...
Start with either transport. For wireless, remove the remote from external power, enable wireless ADB, and run `battery-mon-start.bat`; turn wireless ADB back off after it starts. For USB, connect the remote and run `battery-mon-start.bat`; after collecting and resetting the baseline, the script asks you to disconnect and starts the elapsed clock when the USB device disappears. USB power is allowed only during this setup when USB is the selected ADB transport; cradle, AC, or wireless charging is still rejected. The script enables full wake-lock history when supported and remembers the device's stable hardware serial, transport, and run folder. For the least-biased standby measurement, do not use scrcpy and leave the device in its normal configuration.

After 24–48 hours, restore the transport used to start the run: enable wireless ADB again, or reconnect the same remote by USB. Run `battery-mon-report.bat`; for USB runs it matches the reconnected device by the recorded hardware serial rather than assuming whichever USB device happens to be attached. It writes final battery/power/CPU snapshots, human and machine-readable batterystats, package-to-UID mappings, `REPORT.txt`, and `bugreport.zip` into that run's folder. Load the zip into Battery Historian to correlate drain with packages, processes, wake locks, jobs, alarms, CPU-running time, screen state, and network activity.
