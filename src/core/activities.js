...
function runAction(a, srcTile) {
  if (!a) return;
  /* A DELIBERATE TWO-STEP (v0.70): an action may declare
     `confirm: { key, msg, tone }` — the first press flashes the
     warning (red pulse, TIMING.confirm window), the second fires.
     Built for fallback-routed browse results, whose play EVICTS the
     cast player's queue (design-library-ui.md §5: "never a silent
     hand-off"); generic because the action grammar is shared.
     `srcTile` (v0.70.1) is the element the press landed on, so the
     pulse happens under the finger. */
  ...
  /* the pending-play stamp gets the source tile's NAME (v0.73.3) —
     a trailing ▶ passes its parent tile in srcTile */
  if (a.service === "media_player.play_media" && srcTile) {
    const l = srcTile.querySelector && srcTile.querySelector(".lbl");
    S._playLabel = (l && l.textContent) || "";
  }
...
function firePreset(t) {
  ...
  /* the two-step gate (v0.70), AT TAP TIME — before warm-start,
     before `navigate`, and returning false so a drawer must NOT pop
     while the confirm is pending (the second press needs the tile
     still under the finger). Same key as runAction's gate, so a
     body tap and a trailing ▶ share one pending confirm. */
  if (a.confirm && !barConfirm("act:" + (a.confirm.key || t.id),
      a.confirm.msg, a.confirm.tone,
      document.getElementById("tile_" + t.id))) return false;
  const ref = a.target || a.entity;
  const target = resolveEntity(ref);
  const run = () => {
    const fire = () => {
