...
const ID_SLOTS = { coral: 1, fern: 1, jade: 1, azure: 1, indigo: 1, violet: 1, orchid: 1, rose: 1, slate: 1, firetv: 1, appletv: 1, googletv: 1, samsung: 1, lg: 1, sony: 1, sonos: 1, netflix: 1, spotify: 1, plex: 1, roku: 1, shield: 1, disney: 1, youtube: 1, prime: 1, peacock: 1, paramount: 1, max: 1, hulu: 1, fubo: 1, espn: 1, britbox: 1, pbs: 1 };
/* @identity-slots-end */
/* primary-name aliases (palette canvas V3) — kept OUTSIDE the
   generated block: the generator owns slots, this table owns words */
const ID_ALIAS = { red: "coral", green: "jade", blue: "azure" };
...
function makeTile(t, row, spanCols) {
  ...
  /* IDENTITY (identity-palette V1, 2026-09-01): `identity` names a
     ...
     presets. */
  let acc0 = t.accent != null ? t.accent : t.identity;
  /* primary-name aliases (palette canvas V3, 2026-09-04): red, green
     and blue resolve to their slots — "that is how people describe
     colour". Yellow is refused by design (its hue sits inside the
     reserved focus arc; at our lightness it is olive) and simply
     doesn't resolve — the tile stays unaccented. */
  if (typeof acc0 === "string" && ID_ALIAS[acc0]) acc0 = ID_ALIAS[acc0];
  const idt = typeof acc0 === "string" && ID_SLOTS[acc0] ? acc0 : null;
  const as0 = t.accent_style != null ? t.accent_style : t.identity_style;
  let ids = typeof as0 === "string" ? ID_STYLES[as0] : null;
  /* the Title layout (ids-text: name leads, icon = corner mark) is a
     PRESET-cell design — on any other tile it degrades to its wash
     half, so a section-wide Title style dresses a mixed section
     sanely instead of silently doing nothing on devices (2026-09-03:
     "whats the difference between the icon options and the title
     options" — on a device there wasn't one, which is a bug) */
  if (ids && t.type !== "preset" && ids.indexOf("ids-text") >= 0)
    ids = ids.indexOf("ids-tint") >= 0 ? "ids-tint"
      : ids.indexOf("ids-pbloom") >= 0 ? "ids-bloom" : null;
