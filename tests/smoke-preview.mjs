...
// 4b. the preview REPORTS every landing (v0.79.1 — the soft-remote
//     wash follows the page): navigate posts harmonium_screen
await p.evaluate(() => { window._msgs.length = 0;
  document.getElementById('pv').contentWindow
    .postMessage({ type: 'harmonium_navigate', screen: 'demo' }, '*'); });
await p.waitForTimeout(200);
r.screenReport = await p.evaluate(() => {
  const m = window._msgs.find(x => x && x.type === 'harmonium_screen');
  return m ? m.screen : null;   // expect 'demo'
});
