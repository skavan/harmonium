# Harmonium v0.87.0 — One design language for the whole house

A remote you configure yourself can easily end up looking like it — a patchwork that grew one screen at a time. This release pulls everything under one set of rules. Color and style options let an opinionated design feel like *your* design; every device type now draws with the same control language; icon search finally covers everything installed in your Home Assistant. Around that core: proper controls for the device types that never had them, a page that shows and manages your actual remotes, activities you can drive from anywhere, and a long tail of fixes that came straight from daily use in a real house.
...
<img src="media/v087-accents.png" width="330" alt="A page mixing brand-accented activities (Fire TV running, Spotify, Netflix), title-led presets, and an accented device tile">

*One page wearing the new palette: a running Fire TV activity in its brand orange, Spotify green and Netflix red on their neighbors, title-led presets below, and a plain device tile carrying an accent of its own.*

### Configuration that makes an opinionated design feel bespoke

Activities, presets, apps and now plain device tiles can carry an accent — either one of nine tuned colors (coral, fern, jade, azure, indigo, violet, orchid, rose, slate) or a **real brand color**: Fire TV's orange, Netflix's red, Spotify's green, and twenty more, adjusted only as much as needed to stay visible on a dark tile. Text on top of any of them automatically picks black or white for readability, including on Apple TV's white and Sony's black. Plain color words work too: writing red, green or blue in a config reaches coral, jade or azure, and the picker shows both names.
...
App tiles in the drawer tint their logos with the right brand color out of the box, and stock apps you haven't customized pick this up automatically. Your own hex colors still work exactly as before and are never deleted by trying a named color. And all of it is opt-in: nothing changes on your remotes until you pick an accent.

Every color in the palette is derived, not eyeballed — the same lightness and chroma math tunes all thirty-one, which is why they sit together so well. The full reasoning lives in [the identity palette design notes](design-identity-palette.md).

### A consistent display language across all tiles

Every control shape — sliders, steppers, chips, hold-to-confirm, the two-button pair — is drawn from one shared table that the Studio and the remote both read, so what you configure is what you get, always. The smaller rules got unified too:
...
Why every control looks and behaves the way it does is written down in [the control language design notes](design-control-language.md).

### An icon library that spans your whole install

<img src="media/v087-icon-search.png" width="400" alt="The icon box with door-open typed, answered by the Material set, mdi:door-open, and fa6-solid:door-open at once">
...
Icons still work the same way on the remote: everything a config references is baked into the deployed files at save time, so the remote never fetches an icon pack, ever. And if you use icon packs that only exist as dashboard resources, list their prefixes under **Theme → Icons** and the picker will ask those packs too. How the whole pipeline works — sources, banking, the browser bridge — is in [the icon sets design notes](design-icon-sets.md).
...
<img src="media/v087-controls.png" width="310" alt="A page of the new controls: a switch pair, a hold-to-unlock lock, a number slider, dropdown chips, and a scene button"> <img src="media/v087-dimmer.png" width="310" alt="Two brightness dimmers grouped into one card, with a compact dimmer row below">
...
- **Locks** get the careful treatment they deserve: locking is one tap, but unlocking takes a press-and-hold — half a second, with a fill that grows under your finger, and letting go early cancels. A jammed lock shows in red with a Retry. If your lock can also open a door (a latch), that hold applies there too.
...
<img src="media/v087-remotes.png" width="760" alt="The Your remotes list: three remotes with liveness dots, profiles, versions, battery levels, and Identify and Reload buttons">
...
All of this was built to cost the battery nothing: remotes report in on a heartbeat they were already sending, and commands ride the connection they already keep open. The fleet's design — why remotes announce themselves, why profiles are outfits — is in [the remote fleet design notes](design-remote-fleet.md).
...
- Activity, preset, device, doorway and control cards all use the same two tidy rows — name and id up top, icon/accent/style below. A tile drawn as a light or temperature control keeps its accent row; a doorway to another page can wear a color like anything else.
- Menus offer only what their tiles can actually wear — and everything they can: a device tile's style menu doesn't list title-led looks it has no title band for, a section's accent-style menu matches what's in the section, and every control with shapes (light and temperature included) offers its Inline / Compact variant.
- The accent picker is a compact button that opens a two-tab panel — **Accents | Brands** — with every swatch on a small chip so light and dark colors both read, and it now floats above everything instead of getting clipped inside collapsed cards.
  ...
  <img src="media/v087-picker.png" width="290" alt="The Accents and Brands picker open on the Brands tab, each brand color on its own chip">
