...
> **Upgrading from an earlier version? Two things.** Before you update, **export your config** (Studio header → **Export** → *every workspace*) — it takes ten seconds and it's the way back if anything looks wrong afterwards (Import → replace). After the update: restart HA, open the Studio and hard-refresh it (Ctrl+Shift+R), read the status line and the upgrade report, then **Save & Deploy**. The modernizing this release does on load isn't permanent until that save, and your remotes don't see the new engine until then either. Details at the bottom, under *Upgrading*.
...
Four short pictorials walk the new ground with screenshots: [Control groups](https://github.com/skavan/harmonium/blob/v0.87.0/docs/pictorials/Control%20Groups.md) · [Custom controllers](https://github.com/skavan/harmonium/blob/v0.87.0/docs/pictorials/Custom%20Controller.md) · [Entity support and variants](https://github.com/skavan/harmonium/blob/v0.87.0/docs/pictorials/Entity%20Support%20and%20variants.md) · [Styling tiles](https://github.com/skavan/harmonium/blob/v0.87.0/docs/pictorials/Styling%20Tiles.md). The mental model behind controllers, casts and the preview is written up in [How controllers work](https://github.com/skavan/harmonium/blob/v0.87.0/docs/cookbook/how-controllers-work.md).

## Design coherence

The theme of the release: one visual language, applied everywhere, that you can still make your own.

![A page mixing brand-accented activities (Fire TV running, Spotify, Netflix), title-led presets, and an accented device tile](https://raw.githubusercontent.com/skavan/harmonium/v0.87.0/docs/media/v087-accents.png)
...
Streaming App tiles in the drawer are either full color logo tiles (we provide 21 of them, and you can add your own — see [TV app logos](https://github.com/skavan/harmonium/blob/v0.87.0/docs/cookbook/app-logos.md)) or brand colored glyphs, out of the box; stock apps you haven't customized pick this up automatically. Your own hex colors still work exactly as before and are never deleted by trying a named color. And all of it is opt-in: nothing changes on your remotes until you pick an accent.

Every color in the palette is derived, not eyeballed — the same lightness and chroma math tunes all thirty-one, which is why they sit together so well. The full reasoning lives in [the identity palette design notes](https://github.com/skavan/harmonium/blob/v0.87.0/docs/design/notes/identity-palette.md).
...
Why every control looks and behaves the way it does is written down in [the control language design notes](https://github.com/skavan/harmonium/blob/v0.87.0/docs/design/notes/control-language.md).

### An icon library that spans your whole install

![The icon box with door-open typed, answered by the Material set, mdi:door-open, and fa6-solid:door-open at once](https://raw.githubusercontent.com/skavan/harmonium/v0.87.0/docs/media/v087-icon-search.png)
...
Icons still work the same way on the remote: everything a config references is baked into the deployed files at save time, so the remote never fetches a full icon pack. And if you use icon packs that only exist as dashboard resources, list their prefixes under **Theme → Icons** and the picker will ask those packs too. How the whole pipeline works — sources, banking, the browser bridge — is in [the icon sets design notes](https://github.com/skavan/harmonium/blob/v0.87.0/docs/design/notes/icon-sets.md).
...
![A page of the new controls: a switch pair, a hold-to-unlock lock, a number slider, dropdown chips, and a scene button](https://raw.githubusercontent.com/skavan/harmonium/v0.87.0/docs/media/v087-controls.png) ![Two brightness dimmers grouped into one card, with a compact dimmer row below](https://raw.githubusercontent.com/skavan/harmonium/v0.87.0/docs/media/v087-dimmer.png)
...
![The Your remotes list: three remotes with liveness dots, profiles, versions, battery levels, and Identify and Reload buttons](https://raw.githubusercontent.com/skavan/harmonium/v0.87.0/docs/media/v087-remotes.png)
...
![The Accents and Brands picker open on the Brands tab, each brand color on its own chip](https://raw.githubusercontent.com/skavan/harmonium/v0.87.0/docs/media/v087-picker.png)
